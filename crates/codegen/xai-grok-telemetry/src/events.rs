//! Local diagnostic event structs. Every struct needs a `telemetry_event!`
//! binding. `session_id` and `turn_number` are added by `log_event` in the
//! shell integration layer.
//!
//! These structs are shared across binaries (TUI and sampler) without pulling
//! in remote analytics clients. The `CompactionScope` helper that drives paired
//! `compaction_triggered`/`compaction_completed` events stays in shell and
//! calls `super::log_event` directly.
