...
async fn handle_notification(
    ...
    if method == Method::Hook.as_wire_str() {
        ...
        // Never adopt an inbound distributed parent in Anime's local-only build.
        let hook_span = fastrace::Span::root(
            "tool_server.hook",
...
async fn execute_call(
    ...
    // Never adopt an inbound distributed parent in Anime's local-only build.
    let call_span = fastrace::Span::root(
        "tool_server.tool_call",
        fastrace::collector::SpanContext::random(),
    )
    .with_properties(|| {
        [
